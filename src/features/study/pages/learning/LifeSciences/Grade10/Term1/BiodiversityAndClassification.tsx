...
const NEXT_TOPIC_ID = 'five-kingdoms'
          ...
          {view === 'overview' ? (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-10">
              ...
              <div className="space-y-4">
                {/* Topic 1: Biodiversity & Classification — this page */}
                <motion.div onClick={() => setView('interactive-lesson')}
                  ...
                  <div className="flex items-center gap-6">
                    <div className={`shrink-0 w-10 h-10 rounded-[1.25rem] flex items-center justify-center text-sm font-bold ${status === 'mastered' ? 'bg-slate-500 text-white' : 'bg-slate-900 text-white shadow-md shadow-slate-900/10'}`}>
                      {status === 'mastered' ? '✓' : '1'}
                    </div>
                    <div>
                ...
                {/* Topic 2: Five Kingdoms */}
                <motion.div onClick={() => onNavigate('learning-lifesci-g10-t1-five-kingdoms' as AppPage)}
                  className="bg-white rounded-xl border border-slate-200 p-4 flex items-center justify-between gap-6 cursor-pointer hover:border-slate-300 hover:shadow-md transition-all group">
                  <div className="flex items-center gap-6">
                    <div className={`shrink-0 w-10 h-10 rounded-[1.25rem] flex items-center justify-center text-sm font-bold ${nextStatus === 'mastered' ? 'bg-slate-500 text-white' : 'bg-slate-900 text-white shadow-md shadow-slate-900/10'}`}>
                      {nextStatus === 'mastered' ? '✓' : '2'}
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-slate-900 tracking-tight">The Five Kingdoms</p>
                      <p className="text-sm text-slate-400 mt-1">Monera, Protista, Fungi, Plantae, and Animalia.</p>
