...
const TOPIC = {
  ...
  enrichment: {
    outcomes: [
      'Identify the parts of the Excel interface: workbook, worksheet, cell, row, column',
      'Enter and edit data in cells and use correct cell referencing (e.g. A1, B3)',
      'Write basic formulas using arithmetic operators (+, -, *, /)',
      'Use common functions: SUM, AVERAGE, MIN, MAX, COUNT, IF',
      'Format cells and create a basic chart from spreadsheet data',
    ],
    knowledgeChecks: [
      {
        afterStep: 0,
        question: 'In Excel, what is the cell reference for the cell in Column C, Row 5?',
        options: ['5C', 'C5', 'Row5Col3', '3,5'],
        correctIndex: 1,
        explanation: 'Cell references in Excel always use the column letter followed by the row number. Column C, Row 5 = C5. This is called A1 notation. Always column first, then row — never the other way around.',
      },
      {
        afterStep: 2,
        question: 'Which formula correctly calculates the average of cells B1 to B10?',
        options: ['=SUM(B1:B10)', '=AVERAGE(B1:B10)', '=MEAN(B1:B10)', '=AVG(B1,B10)'],
        correctIndex: 1,
        explanation: '=AVERAGE(B1:B10) calculates the arithmetic mean of all values in cells B1 through B10. The colon (:) means "from B1 to B10" (a range). There is no =MEAN or =AVG function in Excel — always use =AVERAGE.',
      },
    ],
    examTip: 'All Excel formulas start with =. Know these six functions for Grade 10 CAT: =SUM(range), =AVERAGE(range), =MIN(range), =MAX(range), =COUNT(range), =IF(condition,value_if_true,value_if_false). The colon in a range (A1:A10) means "from A1 to A10". A comma separates individual cells (A1,A3).',
    summaryPoints: [
      'A workbook is the Excel file; it contains worksheets (tabs); each worksheet is a grid of cells',
      'Cell reference = Column letter + Row number (e.g. B3); always column first',
      'Formulas start with = and use arithmetic operators or functions',
      '=SUM adds; =AVERAGE finds mean; =MIN finds smallest; =MAX finds largest; =COUNT counts numbers',
      '=IF(condition, value_if_true, value_if_false) — the most important conditional function',
      'Charts: select data → Insert → Chart type; used to visualise numeric data',
    ],
  },
}
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => setScratchpadKey(`scratchpad_cat-sheets_q-${idx}`)}
      ...
      <main className="max-w-3xl mx-auto px-4 pt-24 pb-16">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <p className="text-[13px] text-stone-400 mt-0.5">CAT · Grade 10 · Term 1 · Topic 4</p>
          </div>
