...
const RoleButton = ({ role, index, onNavigate }: { role: typeof roles[number]; index: number; onNavigate: (p: Page) => void }) => {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.98 }}
      className="group w-full text-left bg-white border border-brand-border hover:border-accent hover:shadow-lg hover:shadow-accent/8 rounded-2xl px-5 py-4 flex items-center gap-4 transition-all duration-200"
    >
      <div className="w-11 h-11 rounded-xl bg-brand-bg group-hover:bg-accent/8 flex items-center justify-center shrink-0 transition-colors">
        <Icon className="w-5 h-5 text-brand-eyebrow group-hover:text-accent transition-colors" />
      </div>
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[12px] text-brand-eyebrow mt-0.5 font-medium leading-snug">{role.desc}</p>
      </div>
      <ChevronRight className="w-4 h-4 text-brand-border group-hover:text-accent shrink-0 transition-colors" />
    </motion.button>
  );
};
...
export default function PortalEntry({ onNavigate }: { onNavigate: (page: Page) => void }) {
  return (
    <div className="min-h-screen flex flex-col bg-brand-bg">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          ...
          >
            ...
            <p className="text-brand-eyebrow text-[14px] font-medium mb-9">
      ...
      <div className="px-8 py-5 text-[11px] text-brand-eyebrow font-medium border-t border-brand-border">
